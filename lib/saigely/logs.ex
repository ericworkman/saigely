defmodule Saigely.Logs do
  ...
  def list_logs_by_character(%Saigely.Characters.Character{id: id}) do
    from(l in Log, where: l.character_id == ^id, order_by: {:desc, :inserted_at}, limit: 100)
    |> Repo.all()
    |> Repo.preload(:character)
  end

  def list_recent_logs(limit) do
    from(p in Log, order_by: [desc: p.inserted_at], limit: ^limit)
    |> Repo.all()
    |> Repo.preload([:character])
  end
